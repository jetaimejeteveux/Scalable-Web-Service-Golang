...
func dispatchWorkers(db *sql.DB, jobs <-chan []interface{}, wg *sync.WaitGroup){
	for workerIndex := 0; workerIndex <= totalWorker; workerIndex++ {
		go func(workerIndext int, db *sql.DB, jobs <-chan []interface{}, wg *sync.WaitGroup){
			counter := 0
			for job := range jobs {
				doJob(workerIndex, counter, db, job)
				wg.Done()
				counter++
			}
...
func doJob(workerIndex, counter int, db *sql.DB, values []interface{}){
	for {
